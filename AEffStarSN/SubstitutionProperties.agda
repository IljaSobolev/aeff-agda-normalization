...
sub-ren-var : ({Y : Type} (x : Y ∈ Γ) → s' (r x) ≡ V-rename r' (s x)) →
              --------------------------------------------------
              ({Y : Type} (x : Y ∈ Γ) → V-rename (Tl {Y = X}) (s' (r x)) ≡ V-rename (wk₂ r') (V-rename Tl (s x)))
sub-ren-var {s' = s'} {r = r} {r' = r'} {s} H x =
  begin
    V-rename Tl (s' (r x))
...
sub-ren-l {s' = s'} {r = r} H = sub-ren (λ {Hd → refl; (Tl x) → sub-ren-var {s' = s'} {r} H x})

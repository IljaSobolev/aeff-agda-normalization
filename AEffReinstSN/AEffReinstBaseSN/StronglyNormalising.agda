...
open import AEffReinstSN.AEffReinstBaseSN.SubstitutionProperties using (ren-↝↝-Σ)
open import AEffBaseSN.StronglyNormalising using (_∈ₗ_; Hd; Tl; map-∈ₗ; ⊔-∈ₗ-≤; ++-∈ₗ)

open import AEffReinstSN.CoinductiveEffectAnnotations using (decₛ)
...
module AEffReinstSN.AEffReinstBaseSN.StronglyNormalising where
...
reducts-base (match+ (inl V) _ _) = [ r↝ (match+-inl _ _ _) ]ₗ
reducts-base (match+ (inr V) _ _) = [ r↝ (match+-inr _ _ _) ]ₗ
reducts-base _ = []ₗ
...
reducts-complete' (r↝ (match+-inl V M N)) = inj₁ Hd
reducts-complete' (r↝ (match+-inr V M N)) = inj₁ Hd
